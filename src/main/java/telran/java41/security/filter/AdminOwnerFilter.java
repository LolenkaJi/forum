...
public class AdminOwnerFilter implements Filter {
	...
	public void doFilter(ServletRequest req, ServletResponse resp, FilterChain chain)
		...
		if (checkEndPoint(request.getMethod(), request.getServletPath())) {
			...
			String idAuthor = res[3];
			UserAccount user = repositoryUsers.findById(login).get();
			if (!(idAuthor.equals(login) || user.getRoles().contains("Administrator".toUpperCase()))) {
	...
	private boolean checkEndPoint(String method, String path) {
		return path.matches("/account/user/\\w+/?")&& method.matches("DELETE");
	}
}
